...
function gmap_footer($contcomp = false)
{
	global $content,$gmap_config,$SITEURL;
	...
	for($i=0;$i<$maps;$i++){
		...
		if(strpos($contents, 'gmap_'.$gmap_config[$i]['id']) !== false  )
		{
			$add_plugin = true;
			$pagemaps[] = $gmap_config[$i]['id'];
		}
		else
			$pagemaps[] = 0;
		//$pagemaps[] = (strpos($contents, 'gmap_'.$gmap_config[$i]['id']) !== false  )? $gmap_config[$i]['id'] : 0;//(preg_match($patron, $contents)>0) ? $gmap_config[$i]['id'] : 0;
		
		// check components content
		if ($contcomp !== false){
			if(strpos($contcomp, 'gmap_'.$gmap_config[$i]['id']) !== false  )
			{
				$add_plugin = true;
				$pagemaps[] = $gmap_config[$i]['id'];
			}
	...
	if(!$add_plugin) return;
	$gmap_script	= "";
	$gmap_result 	= "\n".'<!-- Google map Plugin : start -->'	
					.'<script language="javascript" type="text/javascript">'
					//.'//<![CDATA['
					.' if (typeof jQuery == "undefined") {'
					. "\n".'  document.writeln(\'<p>Do poprawnego działania wymagane jest jQuery</p>\');'
					. "\n".' };'
					. "\n".' [js]'
					//. "\n".'//]]>'	
					. "\n".'</script>'
			...
			if((string)$pagemaps[$x] == (string)$gmap_config[$i]['id'])
			{
					...
					for($z=0;$z<$a;$z++){
						...
						if(gmap_issetvar($gmap_config[$i]["anchors"][$z]['image'])){
							...
							if(!isset($imgsize[1]) || $imgsize[1]==NULL || $imgsize[1]=="" || $imgsize[1]=="undefined"){ 							
								...
								$GETIMGSIZEJS .= $mkname.'=gmap_getimgSize(\''.$gmap_config[$i]["anchors"][$z]['image'].'\');';														
				...
				if(gmap_issetvar($gmap_config[$i]['icon_image_img'])){
					...
					if(!isset($shasize) || $shasize==NULL  || $shasize[1]=="" || $shasize[1]=="undefined" || $shasize[1]==0){ 					
						...
						$GETIMGSIZEJS .= $shname.' = gmap_getimgSize(\''.$gmap_config[$i]['icon_shadow_img'].'\');';						
				...
				$gmap_script .='  jQuery("#gmap_'.$gmap_config[$i]['id'].'").css({"width":"'.$gmap_config[$i]['width'].'","height":"'.$gmap_config[$i]['height'].'"}).goMap(ops_'.(string)$gmap_config[$i]['id'].');';

				if(gmap_issetvar($gmap_config[$i]['map_styles']))
					$gmap_script .= '$.goMap.setMap({styles:'.$gmap_config[$i]['map_styles'].'});';
			};
		}; 
	}; 
...
function gmap_getXMLdata(){
	...
	if (file_exists($gmap_file)) {
		...
		for($i=0;$i<$c;$i++){
			...
			$gmap[$i]['infotxt']		= stripslashes($x->map[$i]->infotxt);	
			$gmap[$i]['popup']			= $x->map[$i]->popup;
			$gmap[$i]['map_styles']		= stripslashes($x->map[$i]->mapstyles);	
	...
	// default xml data
		...
		$gmap[0]['image']				= "/data/uploads/googlemap/marker.png";
		$gmap[0]['imagepos']			= "cent-bot";
		
		$gmap[0]['shadow']				= "/data/uploads/googlemap/shadow.png";
		$gmap[0]['shadowpos']			= "left-bot";
		
		$gmap[0]['infotxt']				= '<h5>[TITLE]</h5><a href="" target="_blank">Pokaż na większej mapie</a>';
		$gmap[0]['popup']				= 1;
		$gmap[0]['map_styles'] = '';
		$gmap[0]['anchors']			= array();
		$gmap[0]['icon_image_img']  	= "";
		$gmap[0]['icon_image_pos']  	= "cent-bot";
		$gmap[0]['icon_shadow_img']		= "";
...
function gmap_include_check($contcomp = false) {
	...
	// comprueba en contenido de componentes;sprawdza zawartość składnika
	if( $contcomp ){ 
		$contcomp = get_components_content(); 
		if (strpos($contcomp, 'gmap_') === false  ){ $contcomp = false; };
	};
	
	// si no hay mapas en contenido ni en componentes: devulve false; jeśli nie ma mapy w treści lub elementów
	if (strpos($data_index->content, '(%googlemap') === false AND $contcomp === false){  return false; };
	//print_r($data_index->content);
	// todo ok
	add_action('theme-header','gmap_header',$contcomp);
	add_action('theme-footer','gmap_footer',$contcomp);
};
...
function draw_anchor($data_anchor,$i=0)
{
	...
	$gmap_anchor =' '.'<div class="clear"></div>
...
function gmap_show_config() 
{
	...
	if (isset($_POST['submit'])) {
		...
		$xml = @new SimpleXMLExtended('<?xml version="1.0" encoding="utf-8"?><maps></maps>');

		for ($i=0; isset($_POST['gmap_id'][$i]); $i++) {
		
			$nom = $_POST['gmap_id'][$i];
			//print_r($_POST['gmap_map_styles_$nom']
			// RECOGE DATA
			...
			$gmap_config[$i]['map_styles'] 		= gmap_clevar($_POST["gmap_map_styles_$nom"]);
			...
			$item->addChild('address')		->addCData(htmlspecialchars(stripslashes($gmap_config[$i]['address']),ENT_QUOTES,"utf-8"));
			$item->addChild('zoom')			->addCData($gmap_config[$i]['zoom']);
			$item->addChild('maptype')		->addCData($gmap_config[$i]['maptype']);			
			$item->addChild('image')  		->addCData(htmlspecialchars(stripslashes($gmap_config[$i]['image']), 	ENT_QUOTES,"utf-8"));
			$item->addChild('imagepos')		->addCData($gmap_config[$i]['imagepos']);
			$item->addChild('shadow')  		->addCData(htmlspecialchars(stripslashes($gmap_config[$i]['shadow']), 	ENT_QUOTES,"utf-8"));
			...
			$item->addChild('mapstyles')	->addCData($gmap_config[$i]['map_styles']);
			...
			<div class="gmapbox" id="gmapbox_<?php echo $id;?>">		
				...
				<div class="basic">
					...
					<p style="margin:0 15px 0 0;">	
						...
						<textarea name="gmap_infotxt_<?php echo $id;?>"><?php echo stripslashes($gmap_config[$i]['infotxt']); ?></textarea>
					</p>	
				...
				<div class="advanced">
					...
					<p class="clear"></p>
					<fieldset name="STYLES" class="STYLES">
						<legend><?php i18n($gmap_thisfile.'/STYLES');?></legend>	
						
						<label for="PAN">
						<textarea name="gmap_map_styles_<?php echo $id;?>"><?php echo stripslashes($gmap_config[$i]['map_styles']); ?></textarea>
						<?php i18n($gmap_thisfile.'/CTRL_STYLES');?>
						</label>
					</fieldset
				</div>
			</div>
